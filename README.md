...
The service assumes it can reach the Garmin SERV/WDU web socket. On the EuraMobil the SERV connects over the motorhome's internal Ethernet at `172.16.11.7`: the web UI is at `http://172.16.11.7:8888/` and the web socket at `ws://172.16.11.7:8888/ws`. The SERV no longer exposes WiFi (its bootstrap message reports `hasWifi:false`), so connect the Pi to that Ethernet network rather than expecting a SERV WiFi network. The IP may move if the internal network reassigns it, and the Origin header is not required (the SERV only rejects a wrong one). If the configured address is unreachable, the adapter sweeps the local /24 for a host serving the SERV web UI and identifies it by fingerprint rather than by port alone, so a moved address is rediscovered at most once a minute without a redeploy. Xtura is served to clients through Tailscale HTTPS; the Go API binds only to loopback and is not exposed as plain HTTP.
...
Temperature history is stored per sensor under `/var/lib/xtura/sensors/` and
water history beside the active config under `water-history/`. Hourly
compaction keeps ten-minute samples for 30 days and moves older data into
partitioned hourly NDJSON archives retained indefinitely. The live overview
