...
| Runtime mode edits | Mode changes persist to `<config>.runtime.yaml` and immediately apply hardware commands for `off`, `manual`, and `boost`. | `service/runtime/mode.go` |
| Temperature changes | Target changes call `EnsureOn` before stepping temperature. | `heating.Client.SetTargetTemp` |
...
| `POST /v1/heating/target-temperature` | HTTP -> `App.SetTargetTemperature` -> `Client.SetTargetTemp` | `107` or `108` press/release steps | Requires 0.5C increments; waits after each step and detects overshoot. |
...
| `POST /v1/heating/mode/manual` | HTTP -> `SetHeatingModeManual` -> `applyRuntimeMode` | `EnsureOn`, then target set | Persisted before apply; if apply fails the persisted state may already have changed. |
| `POST /v1/heating/mode/boost` | HTTP -> `SetHeatingModeBoost` -> `applyRuntimeMode` | `EnsureOn`, then target set | Rejects non-positive duration; resumes previous mode on cancel/expiry. |
