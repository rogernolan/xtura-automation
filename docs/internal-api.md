...
| `/v1/water/history` | GET | `handleWaterHistory` | `WaterHistory()` | `waterhistory.Document` | `405` | `TestHandleWaterHistoryGet` |
...
| `waterhistory.Document` | seven-day render-ready `chart_samples`, independent fill/empty events, grouped markers, and fresh/grey usage summaries | `service/waterhistory/types.go` | `chart_samples` is an append-only server-side cache: values are rounded to whole percentages, averaged over five minutes, and bucketed hourly for the seven-day chart. Raw sensor samples are retained server-side only for the active five-minute averaging window and are not sent to clients. Identical consecutive readings are discarded, with a one-minute heartbeat retained. Fresh fills are increases; grey empties are decreases. A qualifying event first observed after reconnect is timestamped with service observation time. |
