...
| `waterhistory.Document` | seven-day percentage samples, render-ready `chart_samples`, independent fill/empty events, grouped markers, and fresh/grey usage summaries | `service/waterhistory/types.go` | Samples use 0–100 percentage values. Identical consecutive readings are discarded, with a one-minute heartbeat retained. `chart_samples` is an append-only server-side cache: values are rounded to whole percentages, averaged over five minutes, and bucketed hourly for the seven-day chart. Fresh fills are increases; grey empties are decreases. A qualifying event first observed after reconnect is timestamped with service observation time. |
