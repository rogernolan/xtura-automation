...
| `tracking.FileInfo` | `{"name":string,"bytes":number,"start_time"?:time,"end_time"?:time,"point_count":number}` | `service/tracking/manager.go` | `start_time`, `end_time`, and `point_count` are parsed from each track file; omitted when the file has no points or does not parse. |
