...
| `waterhistory.Document` | seven-day percentage samples, independent fill/empty events, grouped markers, and fresh/grey usage summaries | `service/waterhistory/types.go` | Samples use 0–100 percentage values. Identical consecutive readings are discarded, with a one-minute heartbeat retained. Fresh fills are increases; grey empties are decreases. A qualifying event first observed after reconnect is timestamped with service observation time. |
