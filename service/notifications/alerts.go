...
func (e *Evaluator) evaluateSide(alert Alert, sensorName string, temp float64, at time.Time, side string, limit *float64) []Notification {
	...
	shouldSend := !state.violated && cooldownElapsed
	if state.violated && alert.Mode == DeliveryRepeat && !state.lastSent.IsZero() && !at.Before(state.lastSent.Add(RepeatInterval)) {
		shouldSend = true
	}
	state.violated = true
	if !shouldSend {
